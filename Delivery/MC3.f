...
c simular model dIsing 2D. 
      ...
      program practica 
      implicit none
      integer*4 seed0,nseed,l,i,j,k,imc,ipas,mcd,mcini,seed,c,z
      parameter(l=64)
      integer*4 pbc(0:l+1)
      integer*2 s(1:l,1:l), de1
      integer*8 suma,de,mctot,n
      real*8 mag,magne,genrand_real2,ene,energ,enebis,e_fin,m_fin,ma_fin
      real*8 sum,sume,sume2,summ,summ2,sumam,vare,varm,temp
      real*8 ene_prev,der_e,error_e,error_m,capac,arrel_m,susce
      real*8 w(-8:8)
      character*29 nom,f1,f2,f3,f4
      character*29 fit1(1:4)
...
c construim array per als noms dels fitxers en funció de T 

      f1="MC-L-064-DADES-ENERGIA"
      f2="MC-L-064-DADES-MAG"
      f3="MC-L-064-DADES-SUSCEPTIBILITAT"
      f4="MC-L-064-DADES-CAPACITAT"

      fit1=[f1,f2,f3,f4]

      do z=1,4
            open(UNIT=11,FILE=fit1(z)//".dat")
            close(11)
      end do
      
c Bucle per a les llavors 

      do c=1,40
c Iniciem variables suma
...
c Calcula la magnetització inicial i la energia inicial
...
c Bucle per a una passa de mc
                        ...
                        if ((imc.gt.mcini).and.(mcd*int(imc/mcd).eq.imc) 
     +)then
                              mag=magne(s,l)
                              sum=sum+1
                              sume=sume+ene
            ...
            temp=temp+0.01d0
c Normalització promitjos

            sume=sume/dble(sum)
            sume2=sume2/dble(sum)
            summ=summ/dble(sum)
            sumam=sumam/dble(sum)
            summ2=summ2/dble(sum)

            vare=(sume2-(sume*sume))
            varm=(summ2-(summ*summ))

c Variables restants

            if (ene_prev.ne.0) then
                  der_e=((sume-ene_prev)/dble(n))/0.01d0 
            end if 

            e_fin=sume/dble(n)
            m_fin=summ/dble(n)
            ma_fin=sumam/dble(n)
            error_e=(dsqrt((sume2-sume*sume)/sum))/dble(n)
            error_m=(dsqrt((summ2-sumam*sumam)/sum))/dble(n)
            arrel_m=dsqrt(summ2)/dble(n)
            susce=((summ2-sumam*sumam)/temp)/dble(n)
            capac=((sume2-sume*sume)/(temp*temp))/dble(n)          

c Sortida de dades


            open(UNIT=12,FILE=fit1(1)//".dat",status='old', 
     +action='write',position='append')
            write(12,*) temp,e_fin,error_e
            close(12)

            open(UNIT=13,FILE=fit1(2)//".dat",status='old', 
     +action='write',position='append')
            write(13,*) temp,ma_fin,arrel_m,error_m
            close(13)

            open(UNIT=14,FILE=fit1(4)//".dat",status='old', 
     +action='write',position='append')
            if (ene_prev.ne.0) then
                  write(14,*) temp,capac,der_e
            else
                  write(14,*) temp,capac
            end if
            close(14)

            open(UNIT=15,FILE=fit1(3)//".dat",status='old', 
     +action='write',position='append')
            write(15,*) temp,susce
            close(15)
...
c-----------------------------------------------------------------------
      ...
      implicit none
      real*8 temp,genrand_real2,genrand_real3,ene
      integer*2 s(1:l,1:l)
      integer*8 de,suma
      real*8 w(-8:8)
...
c probabilitat acceptar canvi p=e^-de/T, prob no acceptar 1-p
      ...
      if (de.le.0.or.genrand_real3().lt.w(de)) then
            s(i,j)=-s(i,j)
            ene=ene+de
      end if
