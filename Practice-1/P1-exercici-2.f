c pràctica 1, problema 2 Fenòmens. Genera una matriu S que contindrà la
c dconfiguració de spins. Omplim la matriu amb un conjunt de valors 1 o 
c -1 a l'atzar (50% probabilitat) emprant una distribució de nombres 
c uniformes U(0,1).

      program practica
      implicit none
      integer*4 SEED,L,i,j
      parameter(L=64)
      integer*2 S(1:L,1:L)
      ...
      do i=1,L
            do j=1,L
                  if(genrand_real2().ge.0.5d0) then
                        S(i,j) = -1
                  else if(genrand_real2().lt.0.5d0) then
                        S(i,j) = 1
                  end if
            end do
      end do
