...
c Calcula i mostra magnetització inicial i la mostra per pantalla

      mag=magne(s,l)

      write(*,*) 'Magnetitzacio inicial:', mag

c Calcula energia inicial matriu i la mostra per pantalla

      ene=energ(s,l,pbc)

      write(*,*) "Energia inicial:", ene

c Bucle que repeteix fins a mctot passes 
      do imc=1,mctot
c Bucle per a una passa de mc
            do ipas=1,n
            call mc(s,l,temp,ene,pbc)
            end do
            enebis=energ(s,l,pbc)
            mag=magne(s,l)
            write(*,*) "MC =",imc,"ENERGIA =",enebis,ene,"MAGNE =",mag
      end do
...
c-----------------------------------------------------------------------
      ...
      subroutine mc(s,l,temp,ene,pbc)
      implicit none
